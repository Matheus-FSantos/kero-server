...
public class GetTransactionInfosAdapter implements GetTransactionInfosOutputPort {
    ...
    @Override
    public Transaction get(InputStream file, String contentType) {
        try {
            String base64File = InputStreamUtils.getBase64(file);
            if (base64File == null) throw new RuntimeException("Fail to convert file to base64.");
            ...
            if(!response.getStatusCode().equals(HttpStatus.OK)) {
                log.warn("{} - get - message: gemini api returned non-ok status, status: {}", getClass().getSimpleName(), response.getStatusCode());
                throw new RuntimeException("Internal error.");
            }

            if(response.getBody() == null || response.getBody().steps() == null || response.getBody().steps().get(1) == null) {
            ...
            if(rawJson.contains("```")) {
                log.info("{} - get - message: markdown code blocks detected, cleaning string...", getClass().getSimpleName());
                rawJson = rawJson.replaceAll("```json", "")
                        .replaceAll("```", "")
                        .trim();
            }
            ...
            log.error("{} - get - message: unexpected error processing transaction. e.message: {}", getClass().getSimpleName(), e.getMessage());
            throw new RuntimeException("Internal error.");
        }
    }
}
